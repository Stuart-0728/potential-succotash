...
def register():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            flash('注册成功！我们已向您的邮箱发送了一封验证邮件，请查收并点击验证链接完成验证。', 'success')
        except Exception as e:
            logger.error(f"发送验证邮件失败: {str(e)}")
            flash('注册成功，但发送验证邮件失败，请联系管理员。', 'warning')
        ...
        try:
            ...
            if user and user.verify_password(password):
                ...
                if not user.email_confirmed and not user.is_admin:
                    flash('您的邮箱尚未验证，请查收邮件并点击验证链接。如需重新发送验证邮件，请点击<a href="' + 
                          url_for('auth.resend_confirmation') + '">这里</a>。', 'warning')
...
def confirm_email(token):
    ...
    if current_user.confirm_email(token):
        db.session.commit()
        flash('邮箱验证成功！感谢您的验证。', 'success')
    else:
        flash('验证链接无效或已过期，请重新获取验证邮件。', 'danger')
...
def resend_confirmation():
    ...
    try:
        ...
        flash('发送验证邮件失败，请稍后重试或联系管理员。', 'danger')
