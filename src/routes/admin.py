...
def permanent_delete_activity(id):
    try:
        ...
        log_action('permanent_delete_activity', f'永久删除活动: {activity_title}')
        flash('活动已永久删除（包括所有报名记录）', 'success')
        
        return redirect(url_for('admin.activities'))
    except Exception as e:
        db.session.rollback()
        logger.error(f"Error permanently deleting activity: {e}")
        flash(f'永久删除活动时出错: {str(e)}', 'danger')
        return redirect(url_for('admin.activities'))
...
def api_statistics():
    try:
        ...
        for i in range(5, -1, -1):
            # 计算月份范围
            end_date = datetime.now().replace(day=1) - timedelta(days=1) if i == 0 else (datetime.now().replace(day=1) - timedelta(days=1)).replace(month=(datetime.now().month - i) % 12 or 12)
            ...
            registration_count = Registration.query.filter(
                ...
                Registration.register_time <= end_date
            ).count()
            
            month_label = f"{start_date.year}-{start_date.month}"
...
def activity_registrations(id):
    try:
        ...
        for reg in registrations:
            ...
            student_info = StudentInfo.query.filter_by(user_id=reg.user_id).first()
            
            if user and student_info:
...
@admin_bp.route('/activity/<int:id>/export_registrations')
@admin_required
    ...
    try:
        ...
        for reg in registrations:
            ...
            if user and student_info:
                data.append({
                    '用户名': user.username,
                    '姓名': student_info.real_name,
                    '学号': student_info.student_id,
                    '学院': student_info.college,
                    '年级': student_info.grade,
                    '专业': student_info.major,
                    '手机号': student_info.phone,
                    '报名时间': reg.register_time.strftime('%Y-%m-%d %H:%M'),
                    '状态': '已报名' if reg.status == 'registered' else '已取消'
                })
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df.to_excel(writer, index=False, sheet_name='报名信息')
        
        output.seek(0)
        
        # 设置响应头
        filename = f"{activity.title}_报名信息_{datetime.now().strftime('%Y%m%d%H%M%S')}.xlsx"
        return send_file(
            output,
            as_attachment=True,
...
def export_students():
    try:
        ...
        for user in students:
            ...
            if student_info:
                # 获取该学生参与的活动数量
                activity_count = Registration.query.filter_by(user_id=user.id, status='registered').count()
                ...
                data.append({
                    ...
                    '专业': student_info.major,
                    '手机号': student_info.phone,
                    '邮箱': student_info.email,
                    '注册时间': user.created_at.strftime('%Y-%m-%d %H:%M'),
                    '参与活动数': activity_count
                })
