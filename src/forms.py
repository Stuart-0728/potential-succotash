...
class ActivityForm(FlaskForm):
    title = StringField('活动标题', validators=[DataRequired(message='活动标题不能为空'), Length(max=100)])
    description = TextAreaField('活动描述', validators=[DataRequired(message='活动描述不能为空')])
    location = StringField('活动地点', validators=[DataRequired(message='活动地点不能为空'), Length(max=100)])
    ...
    status = SelectField('活动状态', choices=[('active', '活动中'), ('completed', '已结束'), ('cancelled', '已取消')], default='active')
    poster = FileField('活动海报', validators=[FileAllowed(['jpg', 'png'], '只允许上传jpg和png格式的图片')])
    is_featured = BooleanField('设为重点活动', default=False)
    points = IntegerField('活动积分', validators=[NumberRange(min=0, max=100, message='积分必须在0-100之间')], default=10)
    tags = SelectMultipleField('活动标签', coerce=int)
    submit = SubmitField('保存')

    def __init__(self, *args, **kwargs):
        super(ActivityForm, self).__init__(*args, **kwargs)
        self.points.description = '学生参加活动获得的积分值，默认普通活动10分，重点活动20分'
