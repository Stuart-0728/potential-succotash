...
class User(db.Model, UserMixin):
    ...
    # 邮箱验证相关字段
    email_confirmed = Column(Boolean, default=False)  # 邮箱是否已验证
    confirmation_token = Column(String(128))  # 验证令牌
    confirmation_token_expires = Column(DateTime)  # 令牌过期时间
    ...
    def generate_confirmation_token(self, expiration=86400):
        ...
        # 设置过期时间
        self.confirmation_token_expires = datetime.now(pytz.UTC) + timedelta(seconds=expiration)
    ...
    def confirm_email(self, token):
        ...
        # 检查令牌是否过期
        if self.confirmation_token_expires < datetime.now(pytz.UTC):
            return False
