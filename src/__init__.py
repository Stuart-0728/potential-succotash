...
def create_app(config_name=None):
    ...
    def before_request():
        ...
        if current_user.is_authenticated and not current_user.is_admin:
            # 排除不需要验证邮箱的路由
            excluded_routes = ['static', 'auth.logout', 'auth.confirm_email', 
                              'auth.resend_confirmation', 'main.index']
            
            if (not current_user.email_confirmed and 
                request.endpoint not in excluded_routes and
                not request.path.startswith('/static/')):
                
                # 允许访问首页、登出和邮箱验证相关页面
                flash('请先验证您的邮箱后再继续使用系统功能。如需重新发送验证邮件，请点击<a href="' + 
